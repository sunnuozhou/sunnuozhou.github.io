...
因为记忆化搜索不太好优化，我们先把记忆化搜索转化为DP。现在我们的状态为$(l,i,r)$，转移时枚举了$(j,k,p,q)$。因为DP转移依赖性的必要，先按$r-l+1$的大小枚举$(l,r)$。考虑优化DP，转移时枚举$(p,q)$，因为原DP转移是$F(l,i,r)+=F(l,j,p)*F(q,k,r)*F(p+1,i,q-1)$，转移条件为$l\le j\le p< i < q \le k\le r，A_{j,k}=1$，所以考虑记$Fs(l,x,r)$表示$\sum_{A_{x,i}=1}{F(l,i,r)}$,计算$S=\sum F(l,j,p)*F(q,k,r)=\sum Fs(l,k,p)*F(q,k,r)$。计算时只需枚举$k$。
